...
module.exports = async ({ n, semester, location, log, buildTRMAPI }) => {
  ...
  for(let i = 0; i < n; i++) {
    ...
    if(state === 'LEARNER') {
      ...
      techie.edyoucatedHandle = faker.internet.userName(firstName, lastName)
  ...
  for(const techie of techies) {
      ...
      query: `
        mutation(
          ...
          insert_techies_one(object: {
            ...
            edyoucated_handle: $edyoucatedHandle,
    ...
    if(techie.state === 'LEARNER') {
      ...
      for(const weekDelta of [-3, -2, -1, 0]) {
        const week = moment(now).add(weekDelta, 'weeks')
        await trmAPI.updateTechieActivity({
          techieID: id,
          year: week.toObject().years,
          week: week.week(),
          type: 'edyoucated',
