...
module.exports = async ({graphqlURL, token, fetch, log}) => {
  const { queries, mutations, domainData } = await buildRepo()
  ...
  return {
    ...
    getEdyoucatedTeamMembers: async ({ edyoucatedTeamID }) => {
      const data = await fetchQuery({
        query: queries.fetch('GET_EDYOUCATED_TEAM_MEMBERS'),
        variables: { teamID: edyoucatedTeamID }
      })

      if(
        !data.edyoucated_teams_by_pk ||
        !Array.isArray(data.edyoucated_teams_by_pk.members)
      ) {
        return Promise.reject({
          reason: `GraphQL API responded with an invalid result`,
          error: data
      ...
      return data.edyoucated_teams_by_pk.members
    },
    ...
    updateTechieActivity: async ({ techieID, semesterID, semesterWeek, type, value, edyoucatedImportedAt, edyoucatedNextImportAfter }) => {
      const data = await fetchQuery({
        query: mutations.fetch('UPDATE_TECHIE_ACTIVITY'),
        variables: { techieID, semesterID, semesterWeek, type, value, edyoucatedImportedAt, edyoucatedNextImportAfter }
      })
    ...
    createTeamMember: async ({ location, email, firstName, lastName }) => {
      const data = await fetchQuery({
        query: mutations.fetch('CREATE_TEAM_MEMBER'),
        variables: {
          location,
          email,
          firstName,
      ...
      if(!data.insert_team_members_one) {
        return Promise.reject({
          reason: `GraphQL API responded with an invalid result`,
          error: data
        })
      ...
      // returns { id: "UUID", functional_team: 'TEAM' }
      return data.insert_team_members_one
    },
    getUsedTechieKeys: async ({ techieKeys }) => {
      const data = await fetchQuery({
        query: queries.fetch('GET_USED_TECHIE_KEYS'),
        variables: {
          techieKeys
        }
      })
      ...
      // returns ['LOC://KEY1', 'LOC2://KEY2']
      return data.techies.map(t => t.techie_key)
    },
    getProjectByNameAndSemester: async ({ name, semesterID }) => {
      const data = await fetchQuery({
        query: queries.fetch('GET_PROJECT_BY_NAME_AND_SEMESTER'),
        variables: {
          name,
          semesterID,
        }
      ...
      if(data.projects.length > 1) {
        log.warning(`Multiple results returned for project ${name} in semester ${semesterID}, chose the first one`, { name, semesterID })
      }

      // returns { found: true, project: { id: '...' } }
      return {
        found: true,
    ...
    terms: {
      byID: id => {
        if(id in domainData.terms) {
          return domainData.terms[id]
        }
    ...
    tracks: {
      byID: id => {
        if(id in domainData.tracks) {
          return domainData.tracks[id]
        }
    ...
    locations: {
      byID: id => {
        if(id in domainData.locations) {
          return domainData.locations[id]
        }
